class BtsController < ApplicationController
  ...
  def show
    ...
    @results = Counts.meeting_specific_criteria(@start_month, @start_year, @end_month, @end_year, @airline&.id, @aircraft_type&.id, @departure_airport&.id, @arrival_airport&.id, @groups)
  end
end
